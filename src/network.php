...
class CNetwork
{
    ...
    const HTTP_HHT_FORWARDED_FOR			    = 'HHT-Forwarded-For';

    //	errors
    const ERROR_SUCCESS				            = 0;
    const ERROR_NETWORK_CURL_NOT_SUPPORTED      = 101;
    const ERROR_NETWORK_CURL_INIT			    = 102;
    const ERROR_NETWORK_PARAMETER			    = 103;
    const ERROR_NETWORK_REQUEST_METHOD		    = 104;
    const ERROR_NETWORK_HTTP_STATUS			    = 105;

    const ERROR_INVALID_HTTP_HEADER			    = 106;
    const ERROR_INVALID_SERVER_ADDR_EMPTY		= 107;
    const ERROR_INVALID_SERVER_ADDR_UNDEFINED	= 108;
    const ERROR_INVALID_REMOTE_ADDR_EMPTY		= 109;
            ...
            if ( is_array( $_SERVER ) )
            {
                    ...
                    is_string( $_SERVER[ 'SERVER_ADDR' ] ) )
                {
                    ...
                    if ( strlen( $sServerAddr ) > 0 )
                    {
                        ...
                        if ( array_key_exists( self::HTTP_HHT_FORWARDED_FOR, $_SERVER ) &&
                            is_string( $_SERVER[ self::HTTP_HHT_FORWARDED_FOR ] ) )
                        {
                            $sXscnForwardedFor = trim( $_SERVER[ self::HTTP_HHT_FORWARDED_FOR ], "\r\n\t, " );
                        }
                        ...
                        if ( is_string( $sXscnForwardedFor ) && strlen( $sXscnForwardedFor ) > 0 )
                        {
                            ...
                            $arrOptHttpHeader[ self::HTTP_HHT_FORWARDED_FOR ]	= $sNewXscnForwardedFor;
                        }
                        else if ( is_string( $sRemoteAddr ) && strlen( $sRemoteAddr ) > 0 )
                        {
